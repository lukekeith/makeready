...
//
//  NAME DEVIATION: the registry name for C-045 is `TextInput`, but
//  `MakeReady/Components/Input/TextInput.swift` already declares `struct TextInput` in this
//  same module and is a legacy file this lane may not touch (preview-build.md §3 rule 2).
//  The type therefore carries a `UI2` prefix. Reported by the build run.
//
...
struct UI2TextInput: View {
    ...
    var body: some View {
        contentRow
            ...
            .padding(.horizontal, UI2Token.Space.cardPadding)
            ...
            .overlay(
                ...
                RoundedRectangle(cornerRadius: UI2Token.Radius.cardSm)
                    .strokeBorder(focused ? UI2Token.textPrimary : UI2Token.cardBorder, lineWidth: 1)
            )
    }
    ...
    // docs/ui2/design-system/tokens.md, so it cannot resolve to a UI2Token member
    ...
    private var contentRow: some View {
        Text(text.isEmpty ? placeholder : text)
            ...
            .ui2TextStyle(UI2Token.TypeStyle.input)
            // §2: placeholder `color-input-placeholder`, value `color-input-value`.
            .foregroundColor(text.isEmpty ? UI2Token.inputPlaceholder : UI2Token.inputValue)
            ...
            .overlay(alignment: lines == .single ? .leading : .topLeading) {
                if focused {
                    RoundedRectangle(cornerRadius: UI2Token.Radius.bar)
                        .fill(UI2Token.accent)
