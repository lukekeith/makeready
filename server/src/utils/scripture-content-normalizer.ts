...
/**
 * Canonical formatter for scripture read-block content stored in the database.
 *
 * The iPhone app uses the same logical shape: one verse per markdown line using
 * `N. text`. Keeping this normalization at the server boundary prevents old
 * client formatting quirks — HTML, copied whitespace, escaped newlines, or
 * superscript verse numbers — from becoming the source of truth for new rows.
 */
export function normalizeScriptureMarkdown(input: string | null | undefined): string | null {
  ...
  if (verses.length === 0) {
    return collapseWhitespace(markdownAndHtmlToText(input))
  }
...
export function parseScriptureVerses(input: string): NormalizedScriptureVerse[] {
  ...
  for (let index = 0; index < candidates.length; index += 1) {
    ...
    const rawVerseText = text.slice(candidate.markerEnd, next ? next.markerStart : text.length)
    const cleaned = collapseWhitespace(rawVerseText)
    if (!cleaned) continue
