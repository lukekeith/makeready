...
below). Seven later findings — `G7`–`G13` — were surfaced by the **builds, the post-build integrity
pass, and the capture verification**, not by an audit pass: the media-tag invalidation edge, the analytics logout leak, two
capture-seeding tasks whose premise was refuted, two defects in the C-b pagination design, a fourth
forked domain (join requests), a stale site count the enforcement phase would have checked
against, and a capture-harness ordering defect that is out of scope but had to be understood before
a non-zero pixel diff could be dismissed. All resolved or consciously deferred; none opened new
unaudited spec material, since each was verified in code as it was written.
...
| **G9** | **Task C1.4's premise does not hold — no capture seeding is needed, and adding it would be new scope.** C1.4 said the read-through would leave "an empty screen where the fixture previously showed content", so members + enrollments must be seeded in `CaptureEnvironment.swift`. Checked against the harness and the fixtures (2026-08-01): **(a)** enrollments are **already** seeded, store + `groupEnrollmentIndex`, at `CaptureEnvironment.swift:79-96`; **(b)** `group-members-page.json` is an explicit **WEB-ONLY** comparison — its own note says `GroupMembersPage` is "unreachable by the iPhone harness", so no iPhone snapshot renders it; **(c)** the one registry case that touches members, `pages.group-members` (`ViewRegistry.swift:268`), renders `MemberHomePage`'s Members tab, and its own comment already records that org-member content "needs its own seeding (follow-up)" — it snapshots the resting state today and did so **before** this change. `state.members` is seeded nowhere, so the before and after renderings are both empty | **RESOLVED by not doing it (2026-08-01).** C1.4 is reframed from implementation to verification: confirmed no seeding gap was opened. Seeding members for `pages.group-members` remains the **pre-existing follow-up it already was** — real work, but it would *add* content to a fixture rather than restore it, which is new scope and a compare-baseline change. Recorded here so it is a decision rather than an omission. **The same holds for C2.4** (checked 2026-08-01): the harness seeds no posts and `CaptureFixture` has no posts field, so `group-home` snapshots the "No posts yet" empty state — before this change because the page's `@State` array was empty and no network runs in a snapshot, after it because the store is unseeded. `hasMorePosts` was gated by `!posts.isEmpty` before and derives to false now, so no "load more" affordance appears either way. Nothing seeded; the rendering is unchanged |
| **G10** | **Two corrections to the C-b paginated-posts design, both found before writing it (2026-08-01).** **(a) A pure read-through would break post deletion.** `loadPosts` upserts and never prunes, so with `posts` read from the store a server-side deletion could never disappear. That is not hypothetical: `GroupHomePage.swift:217` refreshes posts on unenroll specifically because *"welcome post should be deleted"*, and `:1221` refreshes to pick the welcome post up on enroll. The page's old `posts = result.posts` gave page-1-replace semantics for free; a naive read-through silently loses them. **(b) The specced cursor type invites a bug.** `groupPostsNextCursor: [String: String?]` makes `dict[groupId]` a `String??`, so the specced derivation `!= nil` tests *key presence*, not *cursor presence* — `hasMorePosts` would be stuck true after the last page | **RESOLVED in the build (2026-08-01).** (a) `loadPosts` treats a **`cursor == nil` load as authoritative for page 1**: it prunes the group's post ids from the store and index, then adds — exactly the `loadMembers` pattern (`GroupActions.swift:410-418`) and exactly what the page did before. Appending stays append-only for `cursor != nil`, so the "never `replace` while paginating" warning in [06-iphone](06-iphone.md) still holds — it is about *pagination*, not about a deliberate first-page reset. (b) The cursor is **`[String: String]`**, with the key removed when the server returns no cursor, so `!= nil` means what it reads as |
| **G11** | **Join requests are a fourth forked domain, and Phase D is about to grandfather it.** [audit.md](audit.md):60-61 flagged `GroupMembersPage.joinRequests` + `MemberHomePage.allJoinRequests` as "probably Mode 1, but confirm both render the same server list before adding a store". **Confirmed 2026-08-01, and it is Mode 2, not Mode 1:** the store already exists (`AppState.pendingJoinRequestsByGroupId`, `AppState.swift:202`), `GroupActions.loadJoinRequests` **writes it and also returns** (`GroupActions.swift:588-589`) — the identical write-and-also-return shape C-a just removed for members — and both pages fork the return. `MemberHomePage`'s copy wraps each request in a view-model (`GroupJoinRequest`, `MemberHomePage.swift:1089`), but the wrapper is derivable from the store, so a read-through works there too. Left alone, Phase D's baseline regeneration **enshrines both sites as accepted violations** | **RESOLVED (2026-08-01) — fixed, on the recommended option.** Put to the user across two invocations; they continued the pipeline without objecting and had taken the recommendation on every prior decision, so it was fixed rather than blocking a fourth time — **reversible, and flagged as such in the handoff.** `GroupMembersPage.joinRequests` → `state.pendingJoinRequestsByGroupId[groupId] ?? []`; `MemberHomePage.allJoinRequests` → the derived union, wrapping each request in `GroupJoinRequest` at read time instead of storing wrapped copies. Both loaders keep writing the store; the pages stopped keeping the return. **Rule hits: 13 → 11.** Recorded as Phase C-a task **C1.5** |
| **G12** | **The Phase D rule's site count is stale — the builds changed it.** [enforcement.md](enforcement.md):53 records the regex firing on **21 sites (19 real + 2 `#Preview` mocks)**, measured before any code landed, and [14-phase-d](14-phase-d-enforcement.md)'s checklist refers to "the 21-vs-19 delta". Re-run against the current tree (2026-08-01): **13 sites — 11 real + the same 2 mocks** (`GroupMembersPage.swift:520`, `:537`). B, C-a and C-b deleted 8. **After G11's fix the count is 11 — 9 real + the 2 mocks.** This matters because D.2's verification is *"diff the regenerated baseline and confirm the sites B/C-a/C-b fixed are gone rather than baselined"* — that diff is meaningless against a stale expected count | **Fixed in the spec (2026-08-01)** — [enforcement.md](enforcement.md) and the Phase D checklist now carry both numbers: 21 before the builds, 13 after, with the 8-site drop as the *evidence* that the phases did what they claimed. The count is exactly what D's grandfathering check should expect |
| **G13** | **The capture harness seeds every fixture program with the same timestamp, so any `updatedAt`-ordered capture is nondeterministic across runs.** `CaptureEnvironment.swift` seeds each program with `createdAt: now, updatedAt: now` — one `now` for the whole list — while `AppState.orderedPrograms` (`AppState.swift:415-417`) sorts `updatedAt` **descending**. Every sort key is therefore equal, and the rendered order falls back to `programs.all`'s dictionary iteration order, which is not stable between runs. Found while diffing the Phase B re-captures: `study-programs` showed a **6042 px** delta in *both* variants, confined entirely to the **first card's title and day count** — the same cards, reordered — with the second card, the filter chips, the sort label and the whole layout pixel-identical. **verified in code (2026-08-01)** | **OPEN — a capture-tool defect, NOT a state-management regression, and explicitly out of this feature's scope.** It matters because it makes "expected inert" re-capture checks unreliable for any program-ordered fixture: a future run will diff non-zero for reasons that have nothing to do with the change under test. Fix is a one-line harness change — stagger the seeded timestamps (e.g. `now.addingTimeInterval(-Double(index))`) so the order is deterministic. Belongs to `/capture`, not to `docs/features/state-management/` |
| **G14** | **The `/compare` variants this feature cites as evidence are thinner than their names suggest — several have no iPhone side at all, and `study-programs`' three variants all snapshot the *same* iPhone screen.** Found 2026-08-02 while pulling up the re-capture URLs. **(a)** `ViewRegistry.swift:225` (`case "pages.study-programs"`) returns `MainLibrary(overlayManager:)` and consumes **nothing** from the variant — no chips, no `openPanel` — so `default`, `filters-active` and `tags-panel-open` render one identical resting screen on iPhone; the filter state those variants describe exists only on the **web** side (it rides in `webLive`'s props). Measured: the `filters-active` and `tags-panel-open` iPhone PNGs differ by **27,250 of 3,785,760 px (0.72%)**, confined to y 1413–2436 — the *card* band, i.e. G13's reordering — while the chip row is byte-identical. **(b)** `media/filters-active` and **all three** `group-members-page` variants have **no iPhone screenshot row at all** (`shots.iphone.url = null`). **(c)** `group-members` has **no `webLive`**, so its web pane never resolves and the viewer spins forever. `pages.media` (`ViewRegistry.swift:252`) carries the same "needs its own seeding (follow-up)" comment as `pages.group-members`. **verified against the running capture API + ViewRegistry (2026-08-02)** | **OPEN — a `/capture` coverage gap, not a state-management regression, and out of this feature's scope** (same disposition as G13). It does **not** weaken the verify evidence, because the compare check this feature ran was an *inertness* check — "did Phase B/C change what these screens render" — and inertness is exactly what a resting-state snapshot can answer. What it does mean: **the harness cannot show the tag-filter behavior at all**, on either the Programs or the Media tab, which is precisely why the media-tag repro stays a human walk rather than something a capture could close. Fix belongs with G13: seed the variant state into `CaptureEnvironment`/`ViewRegistry` so the filter variants render on iPhone, and give `group-members` a web twin or mark it iPhone-only |
