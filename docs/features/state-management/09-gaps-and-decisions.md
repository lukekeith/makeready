...
below). Six later findings — `G7`–`G12` — were surfaced by the **builds and the post-build integrity
pass**, not by an audit pass: the media-tag invalidation edge, the analytics logout leak, two
capture-seeding tasks whose premise was refuted, two defects in the C-b pagination design, a fourth
forked domain (join requests), and a stale site count the enforcement phase would have checked
against. All resolved; none opened new unaudited spec material, since each was verified in code as
it was written.
