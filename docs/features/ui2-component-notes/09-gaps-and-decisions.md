...
**Status: decisions gate closed 2026-09-10 · delta audit (pass 2) clean of blockers — 2 new
findings, both fixed in the spec the same session.** No OPEN blocking row remains; the plan step
is open.
...
| O-1 | O | **Note deletion.** D7 makes notes append-only, so a note written in error can only be retracted by a newer note saying so — which still leaves the mistake in the effective-requirement derivation's input. Does the UI eventually need a "retract" that marks a note inactive (not a delete), and should `/ui2-component-update` honor a retraction marker? | ✅ **ACKNOWLEDGED (2026-09-10)** — append-only ships; revisit when the note volume is real. Not a blocker and never will be |
| O-2 | O | **Per-property supersession is a judgement call.** 03 §3 step 4 says a later statement about "the same property" replaces an earlier one, but two prose notes do not declare which property they are about — the skill infers it. Does the composer eventually need light structure (an optional `property:` prefix, or a tag) so supersession is mechanical instead of inferred? | ✅ **ACKNOWLEDGED (2026-09-10)** — prose + inference ships; the command check C-4 on real notes is the evidence for whether structure is needed |
| O-3 | O | **Unmapped instances.** D5 deliberately omits anything the generator cannot resolve, so parts of a screen will have no box. Is a visible affordance wanted (e.g. the Screen tab listing "N elements unmapped") so the gap is known rather than silently absent? | ✅ **ACKNOWLEDGED (2026-09-10)** — the `.unmapped.json` report (now `capture/scripts/out/`, §G-18) covers the operator; a browsing affordance is a later idea, not a gap in this build |
| O-4 | O | **Notes on a component read from a screen.** A note is global to the component (01 Out of scope), with `@<screen-id>` as the scoping device. If it turns out most notes written from a screen are screen-specific, the model may need a real scope field rather than a convention. | ✅ **ACKNOWLEDGED (2026-09-10)** — the `@<screen-id>` convention ships; the note corpus is the evidence for whether a real scope field is wanted |
| G-1 | G | **The mention index does not include a component's own dependencies as first-class suggestions.** `@` lists everything alphabetically; when annotating `C-063` the useful mentions are usually `C-061` and its hosts. A relevance ranking (dependencies and consumers first) is not specified. | ✅ **ACKNOWLEDGED (2026-09-10)** — alphabetical ships; relevance ranking is a later refinement |
...
### Pass 2 (delta) findings — both fixed in the spec

| # | Row | Status |
|---|---|---|
| G-17 | **The Component tab would be empty for exactly the components a chip can select.** The new tab drew its identity from `elements[].resolved`, but R6's `.cmp-ui2s__chip` lists every row in the spec's §4 closed list while D5 deliberately omits unresolvable instances from the map — so clicking a chip for an unmapped component would select a ref the tab has no record of. Attack that found it: "what does this design assume about the second caller?" — the render click always has a map entry; the chip click frequently does not. | ✅ **Fixed in the spec (2026-09-10)** — 07 §5.1: `fetchUi2Detail(ref)` is the identity source, `elements[].resolved` is a first-paint optimisation only |
| G-18 | **The generator's `.unmapped.json` report had no home.** Written beside the map under `docs/ui2/screens/assets/` it would commit operator scratch into the program's normative tree AND churn `dirStamp(screenAssetsDir)` — the very cache key this feature relies on — on every generator run. | ✅ **Fixed in the spec (2026-09-10)** — 07 §1.1 step 5: `capture/scripts/out/<stem>.unmapped.json`, with the directory gitignored |

Two further traps were caught and written straight into the design rather than raised as findings:
`Ui2Layout` passes the controlled `tab`/`onTab` **only when `isScreen`** (a controlled `component`
tab on a component row would pin the panel to its `comments` fallback and break tab switching on
every 2.0 component), and the mentions `readdir` filters the notes directories' own `README.md`.
The mouseup-selection design was checked against `ZoomPane`'s actual refs: `vpRef` (`:21`, attached
...
**Clear — the plan step is open.** Every `D`/`X`/`C` row is DECIDED or RESOLVED; every remaining
`O`/`G` row is ACKNOWLEDGED (deliberately deferred, never a blocker). The five gate rulings were
taken as **defaults** on 2026-09-10 under an explicit instruction to run the pipeline without
stopping — each is veto-able, and the rows name exactly what to change if one is reversed.

Delta audit pass 2 ran over that new material in the same session (REFERENCE.md §3b) and found two
things, both fixed. Its own consequences were spec-only edits with no new design, so the loop
terminates here.

**Carried into the build as known work, not gaps:** G-2 (the aspect guard wants a fixture built
from a real Figma export — phase 1 task).

## Integrity check — 2026-09-10

**Verdict: SOUND** (0 defects; run after the decisions gate and the delta audit, before the plan
step)

| # | Check | Result |
|---|---|---|
| 1 | Suite completeness | ✅ `README` + `01`–`09` present; `04`/`05`/`06` carry a reasoned **Not affected** |
| 2 | Citations — **33 opened and read** | ✅ every one lands on what the suite says, including the six added by the gate (`ZoomPane.jsx:77-93,104,169`, `Ui2Layout.jsx:94,130,548`, `SidePanel.jsx:25`, `Ui2ScreenTab.jsx:73`, `server.mjs:1181-1184,1543-1546`, `.claude/commands/ui2-component-build.md:105-147`, `.claude/commands/ui2-screen.md:91-168`) |
| 3 | Counts re-run | ✅ 98 mention rows (73 + 25) · 73 registry ids · 12 screen snapshots over 10 specced screens · test baseline 73/72/1 · all 12 PNG dimensions measured for the aspect-guard arithmetic |
| 4 | Commands | ✅ `npm test` = `node --test test/*.test.mjs` (collects `capture/test/*.test.mjs` only) · `npx vite build` resolves (`capture/node_modules/.bin/vite`) · `capture/scripts/` already exists with two sibling scripts · `git status --porcelain docs/ui2 \| grep -v '/notes/'` valid. Routes and the two new CLIs are the things being built, so they are correctly absent — flagged, not passed |
| 5 | Internal consistency | ✅ all 9 intra-suite links resolve · `02`'s ⬜ rows have no work described anywhere else · `01`'s Decisions table and `09`'s D rows agree · no placeholder text |
| 6 | Ledger ↔ artifacts ↔ code | ✅ ledger, README snapshot and 09 all report the same step state after this session's updates; nothing is claimed built (grep: no `ui2-notes.mjs`, no `Ui2ComponentTab.jsx`, no `elements` key on `screen-detail`) |
| 7 | Unverifiable claims | ✅ zero `(claimed — unverified)` markers; every `**verified in code**` stamp in the suite was re-derived this session rather than inherited |
| 8 | Language lint + traceability | ✅ no banned vagueness in `01`–`08` · R1–R13 defined in `01` and all 13 cited in `08` §6, with no orphans on either side · the gate's five rulings are all stated as values or closed lists |

Nothing was skipped except executing the not-yet-built commands, which is by construction.
