...
| D8 | Hit priority | The **smallest** rect containing the point wins, so a `C-043` chip inside a `C-037` row selects the chip. Identical-area ties resolve to the entry that appears **FIRST** in the map, and both producers emit the innermost of an equal-area pair first. **Corrected 2026-09-10 (build phase 4, 09 §G-19):** this row originally said *later*, which is the opposite of the shipped behaviour it claims to reuse — `hitTest` stable-sorts by area ascending and takes `[0]`. Only the iOS harness's deepest-first emission made "later" look right; the screen backfill now sorts (area asc, depth desc) so one rule serves both. | *(default, corrected)* |
