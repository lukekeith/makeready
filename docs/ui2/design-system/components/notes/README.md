...
One append-only markdown file per component, named `C-###.md`. Created the first time a note is
written, so an empty directory means "nobody has written one yet" — not a missing feature.

**Notes are normative.** They say what a component is *for* and how it should behave, which neither
...
- Written by the capture browser's Component tab. Hand-edit only to fix a typo — there is no edit or
