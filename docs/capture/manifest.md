...
- [~] **program-home** ↔ `pages.program-home` (ProgramHomePage, the .programHome modal) — **web twin built + captured** (`ProgramHome.vue`: PageTitle + CoverImagePicker/PublishBadge + TabSlider + CardLesson list + add-day BoxButton; SHARED with the production leader app). Variants: `default` (client+iphone), `enrollments` (client only — iPhone CaptureEnrollment can't seed the tab's group/date data yet), `analytics` (client+iphone, 2026-07-30 — iPhone shows the real Phase-B Analytics tab via `shared.selectedTab` → `ProgramHomePage(initialTab:)` + seeded `AppState.programAnalyticsById`; web side is the honest "Coming soon" stub until the LeaderApp parity pass).
