...
- ✅ Imports UI components from `@/ui` (client path alias)
- ✅ Imports utilities from `@/util` (client path alias)
...
3. **Follow this exact pattern**:
   ...
   import { Button, Icon } from "@/ui"; // Import from ui using path alias
   import { when, useLifecycle } from "@/util"; // Import from util using path alias
...
8. **Commit to client repository**:
   ...
   cd client
   git status
   git add src/pages/[page-name]/
   git commit -m "Add [PageName] page
...
- [ ] Imports from `@/ui` barrel export (not `@/components` or bare `ui`)
- [ ] Imports from `@/util` barrel export (not bare `util`)
...
import { Button } from "ui"; // Also wrong - use path alias
