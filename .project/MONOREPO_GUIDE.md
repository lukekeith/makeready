...
## ⚠️ IMPORTANT: MakeReady Uses Multi-Repo, Not Monorepo

**This guide describes a monorepo pattern, but MakeReady actually uses a MULTI-REPO structure.**

**MakeReady's Actual Structure:**
- **Three separate git repositories**: `/client`, `/server`, `/iphone` (each with own `.git`)
- **NO shared `/ui` or `/util` at root** - these exist only within `/client/`
- **Independent git histories** - each app commits to its own repo
- **No shared code between apps** - ui/util are client-specific

**Key Differences:**

| Feature | Monorepo (this guide) | Multi-Repo (MakeReady) |
|---------|----------------------|------------------------|
| Git repos | One `.git` at root | Three `.git` (client, server, iphone) |
| Shared code | `/ui`, `/util` at root | No shared code at root |
| Commits | Single commit across apps | Separate commit per app |
| UI components | Shared across all apps | Only in `/client/ui` |

**See `.claude/CLAUDE.md` for the correct multi-repo structure and git workflow.**

## Overview

This architecture guide describes **multiple applications in a single repository** (monorepo), allowing you to build full-stack applications with shared code.
